...
**Last Updated:** 2026-05-21 | **Phase:** 2 Complete (Authentication & User Management)
...
### 🔜 Phase 3: Article Scraping Pipeline
- Implement `ScraperService.scrapeSource()` with real RSS parsing
- Celery task queue for background scraping jobs
- `EmbeddingService.generateEmbedding()` via OpenAI API
- pgvector similarity queries in `ClusteringService`

### 🔜 Phase 4: LLM Processing
- `LLMService.generateSummary()` via Anthropic Claude
- Bias detection, country extraction, topic classification
- `EventService.calculateImportance()` scoring

### 🔜 Phase 5: Elasticsearch Search
...
| `LLM_PROVIDER` | `anthropic` | LLMService provider selection |
